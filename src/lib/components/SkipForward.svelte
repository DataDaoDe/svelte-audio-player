<script lang="ts">
  export let onSkipForward: (event: Event) => void;
  export let skipForwardTimeInSeconds: number;

  let message = `skip forward ${skipForwardTimeInSeconds} seconds`
</script>

<button
  title={message}
  aria-label={message}
  on:click|preventDefault={onSkipForward}>
