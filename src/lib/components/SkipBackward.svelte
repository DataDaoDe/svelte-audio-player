<script lang="ts">
  export let onSkipBackward: (event: Event) => void;
  export let skipBackwardTimeInSeconds: number;

  let message = `skip backward ${skipBackwardTimeInSeconds} seconds`
</script>

<button
  title={message}
  aria-label={message}
  on:click|preventDefault={onSkipBackward}>
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24"><path fill="none" d="M0 0h24v24H0z"/><path d="M8 11.333l10.223-6.815a.5.5 0 0 1 .777.416v14.132a.5.5 0 0 1-.777.416L8 12.667V19a1 1 0 0 1-2 0V5a1 1 0 1 1 2 0v6.333z"/></svg>
</button>